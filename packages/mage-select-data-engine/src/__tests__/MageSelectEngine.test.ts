...
describe('MageSelectEngine', () => {
  ...
  it('should perform initial load correctly', async () => {
    ...
    expect(mockFetchPage).toHaveBeenCalledWith(
      1,
      '',
      expect.objectContaining({ searchFields: [], signal: expect.any(AbortSignal) }),
    );
  });
  ...
  it('should pass searchFields to fetchPage', async () => {
    ...
    expect(mockFetchPage).toHaveBeenCalledWith(
      1,
      '',
      expect.objectContaining({ searchFields: ['name', 'email'] }),
    );
  });
  ...
  it('should abort previous requests when searching rapidly', async () => {
    ...
    mockFetchPage.mockImplementation(
      () =>
        new Promise((resolve) =>
          setTimeout(() => resolve({ items: [], hasMore: false }), 100),
        ),
    );
  ...
  it('should initialize with initialSelectedIds and hydrate them', async () => {
    const items = [{ id: '1', name: 'Item 1' }];
    const mockFetchByIds = vi.fn().mockResolvedValue(items);
    const engine = new MageSelectEngine({
      fetchPage: mockFetchPage,
      fetchByIds: mockFetchByIds,
      getId: mockIdGetter,
    ...
    await vi.waitFor(() => {
      expect(engine.getState().selectedItems).toEqual(items);
    });
    expect(mockFetchByIds).toHaveBeenCalledWith(['1']);
  });
  it('should catch AbortError in pagination methods', async () => {
    const engine = new MageSelectEngine({
      fetchPage: mockFetchPage,
      fetchByIds: vi.fn(),
      getId: mockIdGetter,
    ...
    mockFetchPage.mockRejectedValueOnce(abortError);
    await engine.initialLoad();
    expect(engine.getState().isLoading).toBe(false);
    mockFetchPage.mockResolvedValueOnce({ items: [], hasMore: true });
    await engine.initialLoad();
